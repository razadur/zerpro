...
<div class="col-md-6">
    <div class="form-group">
        ...
        <select name="category" required class="form-control" onChange="showUser(this.value)">
            ...
            <?php foreach($get_all_categories as $get_category){ ?><option value="<?php echo $get_category->category_name; ?>"
                <?php if($user_details_info->category == $get_category->category_name){ echo 'selected';}?>
                ><?php echo $get_category->category_name; ?></option>
            <?php } ?>
        </select>
...
<div class="col-md-4">
    ...
    $this->db->where("category_name = '$user_details_info->category'");
    $query_result=$this->db->get();
    $result=$query_result->result();
    $num_rows=$query_result->num_rows();
    $user_details_info->spcialization;
    $spcialization = explode(',',$user_details_info->spcialization);
        for($i=0;$i<$num_rows;$i++){
    ?>
        <div class="checkbox">
            <label>
                <input type="checkbox" value="<?php echo $result[$i]->spcialization?>" name="spcialization[]"
                       <?php if(in_array($result[$i]->spcialization,$spcialization)) echo 'checked';?>
                    ><?php echo $result[$i]->spcialization?>
            </label>
        </div>
    <?php }?>
    </div>
...
<div class="col-md-6">
    ...
    <div class="form-group">
        ...
        <input readonly style="background: #fff" type="Text" class="form-control" id="name" placeholder="" name="country" value="<?php if(!empty($user_details_info->country) )echo $user_details_info->country; else echo 'Puerto Rico' ?>"/>
    </div>
    <div class="form-group">
